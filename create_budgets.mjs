...
async function createCollection() {
  try {
    ...
    try {
      const existing = await pb.collections.getOne('budgets');
      console.log('Budgets collection already exists!', existing.id);
      ...
      if (err.status === 404) {
        ...
        console.log('Budgets collection created successfully!', record.id);
      } else {
        throw err;
      }
    }
